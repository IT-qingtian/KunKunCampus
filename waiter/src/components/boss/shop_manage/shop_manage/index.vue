  ...
  <scroll-view scroll-y style="height: 100vh">
    ...
    >
      ...
      <el-form-item label="营业时间段（自动更改营业状态）">
        ...
        <div>
          <el-checkbox v-model="form.trade_time[0]" size="large" label="周一"></el-checkbox>
          <el-checkbox v-model="form.trade_time[1]" size="large" label="周二"></el-checkbox>
          <el-checkbox v-model="form.trade_time[2]" size="large" label="周三"></el-checkbox>
          <el-checkbox v-model="form.trade_time[3]" size="large" label="周四"></el-checkbox>
          <el-checkbox v-model="form.trade_time[4]" size="large" label="周五"></el-checkbox>
          <el-checkbox v-model="form.trade_time[5]" size="large" label="周六"></el-checkbox>
          <el-checkbox v-model="form.trade_time[6]" size="large" label="周日"></el-checkbox>
        </div>
      </el-form-item>
      <el-divider/>
      ...
      <el-divider/>
      <el-form-item prop="mbp" label="外卖起送价格">
        <div>
          <el-input v-model="form.mbp" @change="change_price" style="width: 120rpx;margin-right: 5px"/>
          <span>元</span>
        </div>
      </el-form-item>
      <el-divider/>
      ...
      <el-form-item label="店铺公告">
        <el-input v-model="form.notice" maxlength="300" type="textarea" show-word-limit rows="5"></el-input>
      </el-form-item>
      <el-divider/>
...
function change_price() {
  form.mbp = Number(form.mbp).toFixed(2)
}

// 修改自动贴合营业时间
function change_ftbh() {
  if (form.ftbh) {
    // 如果尚未设置时间，那就报错
    if (!form.start_time || !form.end_time) {
      // 取消本次操作
      form.ftbh = !form.ftbh
      return uni.showToast({title: "请先设置营业时间段！", icon: "error"})
    }
    // 判定当前时间是否属于营业时间 如果是 那就修改营业状态
    //  切割
...
async function getShopInfo() {
  ...
  form.ftbh = !!form.ftbh
  form.trade_state = !!form.trade_state
