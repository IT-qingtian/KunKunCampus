    ...
    <div class="item">
      ...
      <div class="value">
        <el-checkbox size="large"
                     :disabled="!Boolean( !order.data?.data.take_goods_mode && order.data?.order_over  ===0 )"
...
if (!store_user.authorization) {
  ...
  // 跳转到授权页面
  cfg.goto_authorize(cfg.__authorize_type.boss, {
    callbackUrl: '/pages' + location.href.split('/pages')[1]
  })
}
...
onBeforeMount(async () => {
  const id = props.id
  // 如果没有id 那就返回主页
  if (!id) return uni.navigateTo({url: '/pages/boss/index'})

  // 拉取数据
  const {code, data, msg} = await boss_pull_order(id)

  if (!code) return console.error(msg)
  ...
  if (order.set_remainder) {
    // 之前时间戳
    const before_time = new Date(order.time_qs).getTime()
    // 时间戳间隔(秒)
